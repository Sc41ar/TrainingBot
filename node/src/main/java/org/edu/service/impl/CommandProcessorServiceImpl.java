...
import static org.edu.specifications.OccupationSpecifications.hasOccupationName;
...
public class CommandProcessorServiceImpl implements CommandProcessorService {
    ...
    private String parseAppointment(AppUser appUser, String string) {
        ...
        ArrayList<String> subStrings =
                new ArrayList<String>(List.of(string.split("/appointment |;")));
    ...
    private void processOccupation(AppUser appUser, String cmd) {//ono strajnoe 0_0
        ...
        if (cmd.matches("^(/occupation) (0[1-9]|[12][0-9]|3[0-1])" +
                "\\.(0[1-9]|1[0-2])" +
                "\\.(202[3-9]):([01][0-9]|2[0-3])" +
                "\\.([0-6][0-9]);\"(([a-zA-Zа-яА-Я _+@]+)*)\"$") && rightsCheck) {
    ...
    private String parseOccupation(AppUser appUser, String string) {
        try {
            ...
            if (isThereOccupationThatMatchesDate(parsedDate))
